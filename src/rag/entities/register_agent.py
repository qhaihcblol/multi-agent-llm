...
class RegisterAgent:
    ...
    def _save_chunks(
        self, chunks: list[Chunk], doc_id: str, chunks_storage_dir: str | Path
    ) -> None:
        chunk_data = [
            {
                "id": chunk.id,
        ...
        output_path = Path(chunks_storage_dir) / f"{doc_id}_chunks.json"
    ...
    # Ingest the document
    def ingest(self, doc_path: str, chunks_storage_dir: str | Path) -> str:
    ...
    def _load_existing_nodes(self, storage_path: str | Path) -> list[dict[str, Any]]:
    ...
    def _save_node_metadata(
        self, node: dict[str, Any], storage_path: str | Path
    ) -> None:
        storage_path = Path(storage_path)
